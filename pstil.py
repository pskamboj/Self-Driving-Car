...
def thresholding(img):
    ...
    lowerWhite = np.array([83,0,0])
    upperWhite = np.array([129,255,255])
    maskWhite = cv2.inRange(imgHsv,lowerWhite,upperWhite)
    return maskWhite

def warpImg(img,points,wT,hT,inv = False):
...
def drawPoints(img,points):
    for x in range(4):
        x_coord = int(points[x][0])
        y_coord = int(points[x][1])
        cv2.circle(img, (x_coord, y_coord), 15, (0, 0, 255), cv2.FILLED)
    return img

def getHistogram(img,minPer=0.1,display=False,region=1):

    if region==1:
        histogramValues = np.sum(img,axis=0)
    else:
